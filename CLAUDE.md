...
- `src/exercise.rs` — `Exercise::load` parses an `.asm` file: inline `; ASSERT_REG:` / `; ASSERT_MEM:` / `; ASSERT_FLAG:` directives into `Assertion`s, and the `; I AM NOT DONE` sentinel into `is_done`. An exercise only advances when assertions pass **and** the sentinel is removed.
...
- `src/emulator.rs` — `run_exercise` is the entry point. Sets up Unicorn (`Arch::X86`, `Mode::MODE_16`), maps 64KB (`constants.rs`: `MEM_BASE`/`MEM_SIZE`), loads code at `LOAD_ADDR` (0x0100), `SP=0xFFF0`, runs with a 10k-instruction cap. It dispatches:
  - **legacy path** (`run_legacy_exercise`): runs the whole binary, checks the inline `Assertion`s.
  - **programmatic path** (`run_programmatic_suite`): used when a suite is registered for the exercise name. Runs each test case with its own fresh emulator + `setup` callback. If the suite has a `target_label`, it calls that label as a subroutine (pushes a `0x0000` sentinel return address and stops when `ret` returns to it) — this lets exercises be tested as callable functions with varied inputs.
...
To add an exercise:
1. Add `NN_name.asm` to `template_exercises/` with `; I AM NOT DONE` and either inline `; ASSERT_*` directives (simple) or nothing if it uses a programmatic suite.
2. Optionally register a programmatic suite in `src/exercise_tests.rs` (string name must match the file stem).
